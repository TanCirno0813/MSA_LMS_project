...
public class ChatService {
    ...
    public void saveMessageWithAiResponse(String userId, ChatMessageDto dto) {
        ChatMessageDto userMsg = new ChatMessageDto(UUID.randomUUID().toString(), dto.getMessage(), LocalDateTime.now());
        pushToRedis(userId, userMsg);

        ChatMessageDto aiMsg = new ChatMessageDto(UUID.randomUUID().toString(), generateAiResponse(dto.getMessage()), LocalDateTime.now().plusSeconds(1));
